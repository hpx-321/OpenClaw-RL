#!/bin/bash
# Qwen3.5-4B LoRA training with OpenClaw RL (Binary RL / GRPO)
...
SAVE_CKPT=${SAVE_CKPT:-${REPO_ROOT}/ckpt/qwen35-4b-openclaw-rl-lora}
...
if [ "${USE_WANDB}" = "1" ] && [ -n "${WANDB_KEY_VALUE}" ]; then
  WANDB_ARGS=(
    ...
    --wandb-group qwen35-4b-openclaw-rl-lora
